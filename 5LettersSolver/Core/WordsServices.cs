...
namespace _5LettersSolver.Core
{
    public class WordsServices: IWordsServices
    {
        ...
        public async Task<List<string>> GetNextWords(List<WordResult> wordResults)
        {
            ...
            var words = await GetFilteredWords(requiredChars, forbiddenChars, antiMask, mask);
            ...
            if (unknownChars.Count() > countPossibleCharsToCheck && countKnownChars < 5)
            {
                ...
                return await GetUnknownCharsWords(unknownChars);
            }
            return words;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<string>> GetUnknownCharsWords(HashSet<char> unknownChars)
        {
        ...
        /// <returns></returns>
        public async Task<List<string>> GetFilteredWords(HashSet<char> requiredChars, HashSet<char> forbiddenChars, List<string> antiMask, List<char?> mask)
        {
            var words = await _wordsRepository.GetAll();
